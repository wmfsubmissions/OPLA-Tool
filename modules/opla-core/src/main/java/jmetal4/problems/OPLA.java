...
public class OPLA extends Problem {
    ...
    public void evaluateLinkOverload(Solution solution) {
        LOGGER.info("evaluateLinkOverload()");
        List<jmetal4.experiments.Fitness> fitnesses = new ArrayList<jmetal4.experiments.Fitness>();
        ...
        for (int i = 0; i < fitnesses.size(); i++) {
            // printar%
            System.out.println("ANTES: " + fitnesses.get(i).getValue()); ;

        }


        for (int i = 0; i < fitnesses.size(); i++) {
            // calcular o peso de 10%
            double peso = fitnesses.get(i).getValue()*100;
            // aplicar a penalidade nos fitness, com 10% de peso * quantidade de violações
            //solution.setObjective(i, fitnesses.get(i).getValue() + (peso * ((Architecture)solution.getDecisionVariables()[0]).linkOverloadViolation));
            solution.setObjective(i, fitnesses.get(i).getValue() + (peso * ((Architecture)solution.getDecisionVariables()[0]).excedLink));
        }

        for (int i = 0; i < fitnesses.size(); i++) {
            // printar%
            System.out.println("DEPOIS " + solution.getObjective(i));
